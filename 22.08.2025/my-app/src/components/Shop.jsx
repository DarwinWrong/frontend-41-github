...
function init() {
    ...
    return stored ? {cart: JSON.parse(stored), total: caclTotal(JSON.parse(stored))}:initialState
}
...
function reducer(state, action) {
    switch (action.type) {
        case 'add item': {
            ...
            const newCart = exist ? state.cart.map(i => i.id === item.id ? { ...i, quantity: i.quantity + 1 } : i)
...
export default function Cart() {
    ...
    return (
        <div>
            <h2>Товари</h2>
            {products.map(p => (
                <div key={p.id}>{p.name} - {p.price}грн
                    <button onClick={() => dispatch({ type: 'add item', payload: p })}>Додати до кошика</button>
                </div>
            ))}
            <h2>Кошик</h2>
            ...
            {state.cart.map(i => (
                <div key={i.id}>
                    ...
                    <button onClick={() => dispatch({type: 'remove item', payload: i.id})}>Видалити</button>
                </div>
            ))}
            {state.cart.length > 0 && (
                <button onClick={() => dispatch({type: 'CLEAR_CART'})}>Очистити кошик</button>
            )}

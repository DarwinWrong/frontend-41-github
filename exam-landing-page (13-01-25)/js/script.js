...
function handleSlidesButtons(btn) {
  let ratio = Math.ceil(
    templatesHolder.offsetWidth / templatesHolder.children.length,
  );
...
slideButtons.forEach((btn) =>
  btn.addEventListener('click', () => handleSlidesButtons(btn)),
);
...
const playBtn = document.getElementById('whyUsPlay');

if (wrapper && video && playBtn) {
  const hideIcon = () => wrapper.classList.add('loading');
  const showIcon = () => wrapper.classList.remove('loading');

  if (video.readyState >= 2) {
    showIcon();
  } else {
    video.addEventListener('loadeddata', showIcon, { once: true });
  }
  ...
  video.addEventListener('pause', showIcon);
  video.addEventListener('ended', showIcon);
  video.addEventListener('waiting', hideIcon);
  video.addEventListener('seeking', hideIcon);
  video.addEventListener('playing', hideIcon);
}
